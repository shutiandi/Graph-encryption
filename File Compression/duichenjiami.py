    ...
    with open(file=readfilename,mode="rb") as filerd,open(file=writefilename,mode="wb") as filewd:
        while True:
            ...
            for i in range(1, readnum):
                numlist.append(int(numl[-i]))
            numlist.append(int(numl[0]))
            for i in range(readnum):
                filewd.write(numlist[i].to_bytes(byteorder="little",length=1))
            # print(numlist)
            # break
...
def duichenjiemi(jiamifile,jiemifile,num=7):
    readnum = num * 2
    with open(file=jiamifile, mode="rb") as filerd, open(file=jiemifile, mode="wb") as filewd:
        while True:
            numl = filerd.read(readnum)
            if len(numl) < readnum:
                filewd.write(numl)
