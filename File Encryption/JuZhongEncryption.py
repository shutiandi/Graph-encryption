...
def Toset(list):
    ...
    for i in list:
        if i not in rlist:
            rlist.append(i)
    return sorted(rlist)
filename="img103_chognfu.jpg"
jiamiwrfilename="img103_chognfu.jpg.jiami.txt"
jiemiwrfilename1="img103_chognfu.jpg.jiemi.jpg"
...
with open(file=filename, mode="rb") as filerd, open(file=jiamiwrfilename, mode="wb") as filewd, open(file=jiamiwrfilename+"juzhong.dat", mode="wb") as filedat:
    while True:
        numtree = filerd.read(127)
        if len(numtree)<127:
        ...
        if len(tset)<=2:
            ...
            zhong = tset[len(tset) // 2]
            a = zhong - tset[len(tset) // 2 - 1] - 1
            b = tset[len(tset) // 2 + 1] - zhong - 1
            if a > b:
                bh = b
            elif a == b:
                bh = b
            ...
            zhong = tset[len(tset) // 2 + 1]
            a = zhong - tset[len(tset) // 2] - 1
            b = tset[len(tset) // 2 + 2] - zhong - 1
            if a > b:
                bh = b
            elif a == b:
                bh = b
            ...
            if a == 0 or b == 0:
                bh = 0
        filedat.write(int(bh).to_bytes(length=1, byteorder="little"))
        sstlist1 = []
        for i in sstlist:
            if zhong==0:
                sstlist1.append(i)
...
with open(file=jiamiwrfilename, mode="rb") as filerd, open(file=jiemiwrfilename1, mode="wb") as filewd,open(file=jiamiwrfilename+"juzhong.dat", mode="rb") as filedat:
    while True:
        numtree = filerd.read(127)
        if len(numtree)<127:
            filewd.write(numtree)
        ...
        for i in numtree:
            sstlist1.append(i)
        bh1=0
        bh1 =fixnumdict[filedat.read(1)]

        zhong1 = 0
        estlist = sstlist1
        tset1 = Toset(sstlist1)
        if len(tset1)<=2:
            zhong1=0
        elif len(tset1) % 2 == 0:
            zhong1 = tset1[len(tset1) // 2]
        else:
            zhong1 = tset1[len(tset1) // 2 + 1]
...
#  27M txtfile 压缩率为27.1，原txt压缩率为23.1  可作为加密方式。
