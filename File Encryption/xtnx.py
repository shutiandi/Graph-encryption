...
jiamiwrfilename="img103_chognfu.jpg.3t1xjiami.txt"
jiemiwrfilename1="img103_chognfu.jpg.3t1xjiemi.jpg"
...
with open(file=filename, mode="rb") as filerd, open(file=jiamiwrfilename, mode="wb") as filewd:
    while True:
        ...
        if numa>numb:
            ...
            numx=-1
        filewd.write(int(numa).to_bytes(length=1, byteorder="little"))
        filewd.write(int(numb).to_bytes(length=1, byteorder="little"))
        numc = (numtree[2] + numx) % 256
        filewd.write(int(numc).to_bytes(length=1, byteorder="little"))
...
with open(file=jiamiwrfilename, mode="rb") as filerd, open(file=jiemiwrfilename1, mode="wb") as filewd:
    while True:
        ...
        numc=(numtree[2]+numx)%256
        filewd.write(int(numc).to_bytes(length=1, byteorder="little"))
#  27M的txtfile 加密file压缩率为27.8，原txt压缩率为23.1  可作为一种加密方式。
